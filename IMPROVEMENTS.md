...
- **Error Handling**: Add try-catch blocks around API calls (e.g., bug report submission) and provide user-friendly error messages.
- **Type Safety**: The TypeScript setup is good, but add more strict types for props and state (e.g., in `RulesWiki.tsx`, the `selectedItem` state could be more strongly typed).
- **Performance**: The app loads large data arrays on import. Consider lazy loading or code-splitting for rules data if it grows.
- **Accessibility**: Add ARIA labels and keyboard navigation support (e.g., for modal close buttons, search input).
- **Testing**: No tests present. Add unit tests for components and integration tests for key features using Vitest or Jest.
- **Linting**: The lint script uses TypeScript checking, but consider adding ESLint for code style consistency.
- **Environment Variables**: The `.env.example` is good, but add validation for required vars in the server.
...
- **Expanded Reference Sections**: Implement Phase 4 features:
    - **Spell Book**: Add a new tile with categorized spells (e.g., by school/element).
...
- **User Experience**:
    - Add a loading state for mission/rule details.
    ...
    - Improve mobile responsiveness (e.g., better modal sizing on small screens).
...
- **Server Improvements**:
    ...
    - Add validation for bug report content (e.g., minimum length).
- **Styling & Theming**:
    ...
    - Add more visual elements (e.g., icons for different rule categories).
...
- **Documentation**: Update `README.md` with more detailed setup instructions (e.g., SMTP config steps).
- **Versioning**: The version is hardcoded in `App.tsx` (v1.0.4). Consider automating this with a build script.
